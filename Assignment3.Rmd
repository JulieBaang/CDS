...
I filter the data for only suicide attacks, code a gender variable from the summaries, and create my own outcome variables. This gives me three different measures of lethality: 'nkill', 'nkillwound', and 'lethality' (ratio).
...
d$nkillwound <- (d$nkill+d$nwound) # creating outcome variable 'nkillwound' as total casualties
...
ggplot(d, aes(x = iyear, fill = female)) +
  ...
  labs(
    ...
    y = "Number of Attacks",
    fill = "Gender",
    title = "Histogram of Yearly Attacks") +
  scale_fill_manual(
...
Visually inspecting the data, it appears highly unbalanced. Recent years, male perpetrators, and certain geographical regions are over-represented.
...
```{r}
cor.test(d$iyear, d$lethality) # Running a correlation on year and lethality
```

Judging from the correlation results, there is a weak but significant (p < .001) correlation between year and lethality.

## t-test for testing gender-lethality relationship (unequal mean lethality between genders)

```{r}
t.test(nkill ~ female, data = d, var.equal = FALSE) # using Welch's t-test because of unequal variances
t.test(nkillwound ~ female, data = d, var.equal = FALSE)
t.test(lethality ~ female, data = d, var.equal = FALSE)
```

Neither of the lethality measures turned out to have significant different means between gender groups. Also, as I'm doing multiple t-tests, the threshold should actually be corrected for multiple comparisons anyways. This confirms my suspicion from the boxplot, that gender alone does not predict lethality.
...
Baseline indicates male (female = 0), year = 1970, and region = Australasia & Oceania.

```{r}
summary(m6)
```

Inspecting the model summary, increasing year and female perpetrators significantly increase attack lethality (p < .001, and p < .01, respectively) compared to the baseline. The effect sizes appear to be .01 increase in lethality for every additional year, and .07 increase in lethality for being female. Furthermore, there is a negative interaction effect of certain geographical regions (Middle East & North Africa; and South Asia) on gender. This indicates that the effect of gender on lethality varies depending on where the attack takes place. Interestingly, even though female perpetrators. However, the model has a very low R-squared value, indicating that the model only explains a fraction of the variance in data. In other words, attack lethality depends more on other factors than the ones investigated here.
